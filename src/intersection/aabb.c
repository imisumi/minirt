...
/*   Updated: 2024/02/16 18:30:13 by imisumi-wsl      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	aabb_intersection_f(t_rayf ray, t_aabb aabb)
{
	...
	while (i < 3)
	{
		...
		{
			if (!aabb_check_intersection(&tnear, &tfar, \
				(aabb.min_f[i] - ray[ORIGIN][i]) / ray[DIR][i], \
				(aabb.max_f[i] - ray[ORIGIN][i]) / ray[DIR][i]))
				return (0);
		}
		i++;
	}
